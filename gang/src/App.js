...
const contractAddress = '0xC09905F5656dF6a1B51D41Da2232eDD00a60186C';
...
const NFT = () => {
    ...
    const fetchContractData = useCallback(async () => {
        try {
            ...
            const priceValue = await contract.price(); // Retrieve the price value
    ...
    const handleMintTokens = async () => {
        try {
            if (contract && numTokensToMint > 0) {
                const mintedPerWallet = await contract.mintedPerWallet(
    ...
    return (
        <div>
            <h1>NFT Contract</h1>
            <p>Sale Active: {isSaleActive ? 'Yes' : 'No'}</p>
            <p>Total Supply: {totalSupply}</p>
            <p>Max Tokens: {maxTokens}</p>
            <p>Max Mint Per Tx: {maxMintPerTx}</p>
            <p>Price: {price}</p>
            <form onSubmit={handleMintTokens}>
                <label>
                    ...
                    <input
                        ...
                        onChange={(e) => setNumTokensToMint(parseInt(e.target.value))}
                    />
                </label>
